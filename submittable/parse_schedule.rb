...
class ParseSchedule
  ...
  COLS =
      ['Performer', '2nd Performer', '3rd Performer', '4th Performer', '5th Performer',
       'Fort', '2nd Fort', 'Event Identifier',
       'Event Name', 'Start Time', 'End Time', 'Venue', 'Description', 'Kidfort Approved', 'All Ages', 'Cancelled',
       'Event ID', 'Performer ID', '2nd Performer ID', '3rd Performer ID', '4th Performer ID', '5th Performer ID']
  ...
  def run
    ...
    (HEADER_ROW + 1..ws.max_rows).each do |row|
      ...
      e[:performers] = []
      e[:performers] << get_performer(ws, row, '')
      ...
      e[:start_time]        = get_relative_time(ws, row, 'Start Time', day_offset).iso8601
      e[:end_time]          = get_relative_time(ws, row, 'End Time', day_offset).iso8601
      e[:venue]             = get_required_string(ws, row, 'Venue')
      e[:description]       = get_optional_string(ws, row, 'Event Name')
  ...
  def get_relative_time(ws, row, col_name, day_offset)
    ...
    time = time + (event_time.hour * 60 * 60) + (event_time.min * 60)
    time
  end
