...
describe('PlaylistController', function() {
    describe('#get', function() {
        it('Creates an object hash of playlists that contains a playlist keyed at 1', function(done) {
            ...
            const result = playlistController.get();

            const dynamo = {
                scan(tableParams, queryCallback) {
                    return queryCallback(null, dynamoDbPerformers);
                }
            };
            ...
            assert.lengthOf(result['1'].Tracks, 75), 'Playlist 1 has a length of 76';
        });
    });
});
